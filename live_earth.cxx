		...
		if (scene_ptr) {
			if (li_help[0] == -1) {
				...
				for (int ci = 0; ci < 2; ++ci) {
					li_help[ci] = scene_ptr->add_label("Stick click : Play/Pause \nA button    : Forward/Backward\n\nTrigger     : Select satellite to draw orbit\nGrip button : Deselect satellite",
...
cgv::base::object_registration<live_earth> vr_draw_reg("live_earth");
#ifdef CGV_FORCE_STATIC
cgv::base::registration_order_definition ro_def("vr_view_interactor;vr_emulator;vr_scene;live_earth");
#endif
...
vr_test::vr_test() 
{
	...
	incr = 60;
	cgv::signal::connect(trig.shoot, this, &vr_test::change_time);
}
